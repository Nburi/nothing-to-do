...
1. **Git commits** — commit independently and sensibly after each meaningful step (setup, a finished
   feature, a bugfix). Commit messages in English, precise.
...
The mobile header had accumulated the streak badge, the customizable Header-Badges row, the "Mehr"
dropdown and the avatar/profile dropdown — two near-identical round icon-button triggers sitting close
together on a narrow phone turned out to be the actual source of the "überladen" (cluttered) feeling,
more than the badges themselves. Two structural changes, on top of the badges' own established
horizontal-scroll safety net (unchanged):

- **The wordmark hides below `sm`** (`layouts/app.blade.php`) — the logo icon alone already carries the
  brand, same "text defers to icon on mobile" convention every other header label already followed
  (avatar name, "Mehr" label). Frees real width without losing brand identity at any breakpoint desktop
  users would notice.
- **"Mehr" folds into the avatar menu on mobile.** The standalone "Mehr" trigger+panel is now
  `hidden sm:block` — desktop keeps exactly the two separate dropdowns it always had (a deliberate, still
  valid decision — see the original Header-Badges/"Mehr" docs above). Below `sm`, the avatar button is the
  *only* menu trigger; its dropdown gains a mobile-only (`sm:hidden`) "Weitere Funktionen" section at the
  top, holding the same feature links. Both places render through **`partials/mehr-nav-links.blade.php`**
  (extracted from the old inline `@if` chain) so the desktop panel and the mobile section can never drift
  apart — the same "shared partial" lesson this app has already learned a few times. The partial
  deliberately **recomputes its own show-nav flags from `auth()->user()`** rather than trusting variables
  passed in from the including view, sidestepping the "Blade partial silently overwrites a caller-passed
  variable" trap entirely rather than relying on `??` guards everywhere (see Known Issues).
  - **The emergency dot moves to the avatar button on mobile** (`sm:hidden`, absolute-positioned on the
    circle) — the standalone "Mehr" button's own ambient "you're in an emergency, even with the menu
    closed" signal (desktop, unchanged) has no home once that button hides on mobile, and this is a
    safety-relevant signal that must not silently disappear.
  - **Header-Badges' `max-w` bumped 38vw → 45vw on mobile** — freed real width from the two changes above
    means the badge row needs its own horizontal scroll less often before a realistic badge selection
    fits without it. Still capped, still scrollable — the safety net is unchanged, just less eagerly
    triggered.
  - **Signature moment — "nichts ist weg, nur zusammengeführt."** The first time the newly-combined mobile
    menu opens, its "Weitere Funktionen" links fan in from the avatar circle, staggered ~35ms apart
    (`.header-menu-fan-in` in `app.css`, driven by `partials/mehr-nav-links.blade.php`'s `$stagger` param
    — `true` only for the mobile copy, the desktop panel opens instantly with no flourish). A quick, calm
    proof that folding two triggers into one didn't quietly remove anything. Restarts every time the menu
    reopens for free — Alpine's `x-show` toggles the panel's own `display: none`, and a CSS animation on a
    display:none-d descendant restarts from its 0% state the next time it's shown, no JS bookkeeping
    needed. Respects `prefers-reduced-motion` via the existing global animation-collapse rule.
- Deliberately out of scope for this pass: a scroll-triggered collapsing header (considered as an
  alternative signature moment — bigger engineering surface, harder to verify without a live browser
  session, deferred), and any further reduction of the badges row itself (it already had a working,
  established pattern before this — see Header-Badges above).
...
### A literal directive word inside a `{{-- ... --}}` Blade comment can swallow everything before it
**Symptom:** a brand-new partial (`partials/mehr-nav-links.blade.php`, Mobile Header Redesign) threw
`ErrorException: Undefined variable $mehrShowAgendaNav` at the exact line that used it — even though the
variable was plainly assigned a few lines earlier inside a `@php ... @endphp` block at the top of the same
file. Dumping the *compiled* view from `storage/framework/views/` (not the source `.blade.php`) showed the
compiled PHP was missing its entire top portion — the doc comment, the whole `@php` block, and several
`@if` blocks that came after it — starting only partway through the file, right after the block's closing
`--}}`. `php artisan view:clear` did not change anything; this reproduced fresh on every recompile, so it
was never a stale-cache problem.
**Cause:** the file's opening documentation comment (`{{-- ... --}}`) contained the literal words `@php`,
`@include`, and `@endphp` in its prose (describing what the partial itself does and referencing another
Known Issue by name). Something in this app's Blade compilation pipeline — plausibly an interaction with
Livewire 4's `ExtendBlade` compiler extension, given every compiled `@if`/`@endif` pair in this codebase's
views is wrapped in extra `ExtendBlade::isRenderingLivewireComponent()` bookkeeping — ends up treating a
directive-shaped word found *inside* a `{{-- --}}` comment as a real directive before (or instead of) the
comment being stripped as inert text, rather than the comment being removed as one opaque block first the
way it is in stock Blade. The practical effect: content between the first such literal directive-shaped
word and the *real* directive that legitimately closes it (in this case, a later, unrelated `@endphp`/`@if`
pair) silently vanishes from the compiled output, taking a working `@php` block and several `@if` sections
down with it — with no compile-time error, since from the compiler's confused perspective the result was
still syntactically valid PHP, just missing the variables that block was supposed to define.
**Fix:** never write the literal token `@php`, `@include`, `@endphp`, `@if`, `@foreach` (or any other
Blade directive name preceded by `@`) inside a `{{-- ... --}}` comment in this codebase — rephrase around
it (e.g. "its own inline-PHP block" instead of "its own `@php` block", "included from" instead of
"`@include`d from"). This cost real debugging time because the resulting error (`Undefined variable`, at a
line far from the actual defect) gives no hint that the true cause is a comment several lines *above* it —
when an "undefined variable" error names a variable that is visibly assigned earlier in the same file,
suspect the compiled view before the source: dump `storage/framework/views/<hash>.php` (found via
`grep -rl '<the undefined variable name>' storage/framework/views/`) and check whether the compiled output
is missing content that exists in the `.blade.php` source — if it's short a whole section, look for a
`{{-- --}}` comment mentioning a directive name by its literal `@`-prefixed form anywhere above the gap.
