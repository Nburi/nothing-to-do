<div class="mx-auto flex min-h-[70vh] max-w-md items-center px-4 pb-16 pt-5 sm:px-6">
    <div class="w-full rounded-card border border-line bg-surface p-6 text-center shadow-map">
        @if ($space === null)
            ...
            <p class="mx-auto mt-2 max-w-xs text-[13.5px] leading-relaxed text-ink-faint">
                Der Code <span class="tnum font-medium text-ink-soft">{{ $code }}</span> gehört zu keiner Familie.
                Vielleicht wurde er neu vergeben — frag jemanden aus der Familie nach dem aktuellen Link.
            </p>
            <a href="{{ route('family') }}" wire:navigate class="mt-5 inline-block rounded-card border border-line px-4 py-2 text-sm text-ink-soft transition hover:bg-paper hover:text-ink">
                Zur Familie
            </a>
        @elseif ($alreadyMember)
            ...
            <p class="mt-2 text-[13.5px] text-ink-faint">
                „{{ $space->name }}" ist schon deine Familie hier.
            </p>
            <a href="{{ route('family') }}" wire:navigate class="mt-5 inline-block rounded-card bg-forest px-4 py-2 text-sm font-medium text-white transition hover:brightness-110 active:scale-[0.98]">
                Zur Familie
            </a>
        @else
            ...
            <p class="mx-auto mt-3 max-w-xs text-[13px] leading-relaxed text-ink-faint">
                Wenn du beitrittst, siehst du alle geteilten Aufgaben dieser Familie — und kannst
                selbst welche eintragen und übernehmen. Du bekommst automatisch eine eigene Farbe.
            </p>
            <button
                type="button"
                wire:click="join"
            ...
            <a href="{{ route('family') }}" wire:navigate class="mt-2.5 inline-block text-[13px] text-ink-faint transition hover:text-ink-soft">
                Nein, zurück zur Familie
            </a>
        @endif
    </div>
</div>
