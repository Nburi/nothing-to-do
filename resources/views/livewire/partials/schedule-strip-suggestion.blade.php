...
<p class="truncate text-xs text-ink-faint">
    ...
    @elseif ($suggestion['kind'] === 'planned')
        <button type="button" wire:click="startEdit({{ $suggestion['task_id'] }})" class="text-ink-soft hover:text-ink hover:underline">{{ $suggestion['title'] }} · {{ $suggestion['subtitle'] }}</button>
    @else
