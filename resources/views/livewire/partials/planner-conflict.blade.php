...
<div class="py-2.5 first:pt-0 last:pb-0">
    ...
    <div class="mt-1.5 flex items-center gap-4">
        <a href="{{ route('schedule') }}" wire:navigate class="hit-area inline-flex items-center gap-1 text-xs font-medium text-overprint hover:underline">
        ...
        @if ($item['type'] === 'task')
            <a href="{{ route('app', ['task' => $item['id']]) }}" wire:navigate class="hit-area inline-flex items-center gap-1 text-xs font-medium text-overprint hover:underline">
        ...
        @else
            <a href="{{ route('agenda') }}" wire:navigate class="hit-area inline-flex items-center gap-1 text-xs font-medium text-overprint hover:underline">
                <svg class="h-3 w-3" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M10 3.5 12.5 6 6 12.5l-3 .5.5-3L10 3.5Z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/></svg>
                Deadline ändern
            </a>
        @endif
