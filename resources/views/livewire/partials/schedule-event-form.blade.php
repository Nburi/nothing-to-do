                ...
                @unless ($eventRecurring)
                    <div x-data="{ taskPickerOpen: false }">
                                ...
                                @foreach ($eventLinkedTasks as $picked)
                                    <span wire:key="picked-{{ $picked['id'] }}" class="inline-flex items-center gap-1.5 rounded-full border border-line bg-paper/70 py-1 pl-2.5 pr-1.5 text-xs text-ink">
                                        ...
                                        <button
                                            ...
                                            aria-label="„{{ $picked['title'] }}“ nicht mehr verknüpfen"
                                            class="grid h-4 w-4 flex-none place-items-center rounded-full text-ink-faint transition hover:bg-signal-soft hover:text-signal"
                                        >
                                            <svg class="h-2.5 w-2.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" aria-hidden="true"><path d="M18 6 6 18M6 6l12 12"/></svg>
                                        </button>
                                    </span>
                                @endforeach
                            </div>
                        ...
                        <button
                            ...
                            aria-label="{{ count($eventLinkedTasks) > 0 ? 'Eine weitere Aufgabe mit diesem Eintrag verknüpfen' : 'Eine bestehende Aufgabe mit diesem Eintrag verknüpfen' }}"
                            ...
                            <div class="mt-1.5 max-h-40 space-y-0.5 overflow-y-auto">
                                @forelse ($this->eventTaskCandidates as $candidate)
                                    <button
                                        ...
                                        wire:click="toggleEventLinkedTask({{ $candidate->id }})"
