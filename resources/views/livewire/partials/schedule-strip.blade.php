...
<a href="{{ route('schedule') }}" wire:navigate class="group block">
    <div class="rounded-card border border-line bg-surface px-4 py-3 shadow-map transition hover:border-ink-faint/50">
        @if ($focus)
            @php
                $active = $focus->isActive($now);
                $duration = max(1, $focus->durationMinutes());
                if ($active) {
                    $remaining = $focus->secondsRemaining($now);
                    $total = $duration * 60;
                } else {
                    $startDt = $focus->date->copy()->startOfDay()->addMinutes($focus->startMinutes());
                    $remaining = max(0, (int) $now->diffInSeconds($startDt, false));
                    $total = 300;
                }
                $isTodo = $focus->type === 'todo_session';
                $suggest = $isTodo
                    ? 'To-Dos abräumen'
                    : ($focus->suggestedTask?->title ? 'Vorschlag: '.$focus->suggestedTask->title : 'Fokuszeit');
            @endphp
            <div class="flex items-center gap-4" x-data="focusTimer({ remaining: {{ $remaining }}, total: {{ $total }}, circ: 263.9 })">
                ...
                <div class="min-w-0 flex-1">
                    <div class="mb-0.5 flex items-center gap-2">
                        <span class="rounded bg-forest-soft px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-forest">{{ $isTodo ? 'To-Do-Session' : 'Work-Session' }}</span>
                        <span class="text-[11px] {{ $active ? 'text-forest' : 'text-ink-faint' }}">{{ $active ? 'läuft' : 'gleich' }}</span>
                        <span class="tnum text-[11px] text-ink-faint">{{ $focus->start_time }}–{{ $focus->end_time }}</span>
                    </div>
                    <p class="truncate text-[15px] font-medium text-ink">{{ $suggest }}</p>
                    <p class="truncate text-xs text-ink-soft">{{ $active ? 'verbleibend' : 'beginnt gleich' }} · Zeitplan öffnen</p>
                </div>
                <svg class="h-5 w-5 flex-none text-ink-faint transition group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 18l6-6-6-6"/></svg>
            </div>
            ...
            <div class="relative h-7">
                ...
                @foreach ($events as $e)
                    ...
                    <div
                        ...
                        title="{{ $e->title ?? ($e->isWorkSession() ? 'Session' : 'Pause') }} {{ $e->start_time }}–{{ $e->end_time }}"
                    >
                        @if ($act)
