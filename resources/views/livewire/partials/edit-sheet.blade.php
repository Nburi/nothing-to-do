    ...
    <div class="fixed inset-0 z-50 flex items-end justify-center sm:items-center" role="dialog" aria-modal="true" aria-label="Aufgabe bearbeiten">
        ...
        <div class="animate-rise relative w-full max-w-md rounded-t-2xl border border-line bg-surface p-5 shadow-map sm:rounded-card" @keydown.escape.window="$wire.cancelEdit()">
            <form wire:submit="saveEdit" class="space-y-4">
                ...
                {{-- Notizen: a small toolbar inserts markdown syntax into the textarea;
                     ++text++ is a custom marker (App\Support\Markdown\UnderlineExtension) —
                     standard/GFM Markdown has no underline syntax of its own. --}}
                <div
                    x-data="{
                ...
                >
                    <div class="mb-1 flex items-center justify-between">
                        <label for="editNotes" class="block text-xs font-medium text-ink-soft">Notizen</label>
                    ...
                    @error('editNotes') <p class="mt-1 text-xs text-signal">{{ $message }}</p> @enderror

                    @if ($this->editNotesHtml !== '')
