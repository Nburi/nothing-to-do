{{-- Manage family task lists: invite codes, joining, leaving, your own card
     color. Same sheet shell as agenda-spaces-sheet.blade.php. --}}
<div x-data="{ open: $wire.entangle('showFamilySpaces') }" x-cloak>
    <div
        x-show="open"
        x-transition.opacity.duration.150ms
        @click="$wire.closeFamilySpaces()"
    ...
    >
        <div class="mx-auto max-h-[88dvh] overflow-y-auto rounded-t-2xl border border-line bg-surface p-5 shadow-map md:rounded-card">
            <div class="mb-4 flex items-center justify-between gap-2">
                <h2 class="text-base font-medium text-ink">Familien</h2>
                <button wire:click="closeFamilySpaces" class="grid h-8 w-8 flex-none place-items-center rounded-card text-ink-faint transition hover:bg-paper hover:text-ink" aria-label="Schließen">
            ...
            @forelse ($this->familySpaces as $space)
                @php
                    $isOwner = $space->owner_id === auth()->id();
                    $myColorHere = $space->colorFor(auth()->user());
                    $isActive = $activeSpaceId === $space->id;
                @endphp
                <div wire:key="family-space-{{ $space->id }}" class="mb-2 rounded-card border border-line p-3">
                    ...
                    @if (! $isActive)
                        <button type="button" wire:click="switchSpace({{ $space->id }})" class="mt-1.5 text-[12px] text-ink-soft transition hover:text-forest">
                            Hier anzeigen
                        </button>
                    @else
                        <span class="mt-1.5 inline-flex items-center gap-1 text-[12px] text-forest">
                            <span class="h-1.5 w-1.5 rounded-full bg-forest" aria-hidden="true"></span> Wird gerade angezeigt
                        </span>
                    @endif

                    {{-- Your own card color in this family — a small swatch row, current pick ringed. --}}
                    <div class="mt-2.5">
                        <p class="mb-1 text-[11px] font-medium text-ink-faint">Deine Farbe</p>
                        <div class="flex flex-wrap gap-1.5">
                            @foreach ($colorKeys as $key)
                                <button
                                    type="button"
                                    wire:click="setFamilyColor({{ $space->id }}, '{{ $key }}')"
                                    @class([
                                        'h-6 w-6 rounded-full transition',
                                        \App\Livewire\Support\FamilyColors::bg($key),
                                        'ring-2 ring-offset-2 ring-offset-surface ring-ink' => $myColorHere === $key,
                                    ])
                                    aria-label="Farbe {{ \App\Livewire\Support\FamilyColors::label($key) }} wählen"
                                    title="{{ \App\Livewire\Support\FamilyColors::label($key) }}"
                                ></button>
                            @endforeach
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <div class="ml-auto flex items-center gap-0.5">
                            <button
                                type="button"
                                wire:click="regenerateFamilyInviteCode({{ $space->id }})"
                            ...
                            <button
                                ...
                                @click="if (armed) { $wire.leaveFamilySpace({{ $space->id }}); clearTimeout(_t); armed = false; } else { armed = true; clearTimeout(_t); _t = setTimeout(() => armed = false, 2000); }"
                            ...
                            @if ($isOwner)
                                <button
                                    type="button"
                                    x-data="{ armed: false, _t: null }"
                                    @click="if (armed) { $wire.deleteFamilySpace({{ $space->id }}); clearTimeout(_t); armed = false; } else { armed = true; clearTimeout(_t); _t = setTimeout(() => armed = false, 2000); }"
                                    @click.outside="armed = false; clearTimeout(_t)"
                                    @keydown.escape.window="armed = false; clearTimeout(_t)"
                                    :class="armed ? 'bg-signal text-white' : 'text-ink-faint hover:bg-signal-soft hover:text-signal'"
                                    class="grid h-7 w-7 place-items-center rounded-card transition"
                                    aria-label="Familie {{ $space->name }} für alle löschen"
                                >
            ...
            @empty
                <div class="mb-3 rounded-card border border-dashed border-line p-6 text-center">
                    <p class="text-[13px] leading-relaxed text-ink-faint">
                        Noch keine Familie. Erstell eine und teile den Code — oder tritt mit dem Code
                        eines Familienmitglieds bei.
                    </p>
                </div>
            @endforelse

            <div class="mt-4 space-y-3 border-t border-line pt-4">
                <form wire:submit="joinFamilySpace" class="space-y-1.5">
                    <label for="family-join-code" class="block text-[12px] font-medium text-ink-faint">Einer Familie beitreten</label>
                    <div class="flex gap-2">
                        <input
                            type="text"
                            id="family-join-code"
                            wire:model="familyJoinCode"
                    ...
                    @error('familyJoinCode') <p class="text-xs text-signal">{{ $message }}</p> @enderror
                </form>

                <form wire:submit="createFamilySpace" class="space-y-1.5">
                    <label for="family-new-space" class="block text-[12px] font-medium text-ink-faint">Neue Familie erstellen</label>
                    <div class="flex gap-2">
                        <input
                            type="text"
                            id="family-new-space"
                            wire:model="newFamilySpaceName"
                    ...
                    @error('newFamilySpaceName') <p class="text-xs text-signal">{{ $message }}</p> @enderror
                </form>
            </div>
        </div>
    </div>
