<div class="mx-auto max-w-3xl px-5 py-10 sm:px-6">
    @if ($editingId === null)
        {{-- LIST — every article, draft and published alike. --}}
        <div class="mb-5 flex items-center gap-3">
            ...
            <h1 class="text-xl font-medium text-ink">Bibliothek verwalten</h1>
        </div>
        <p class="mb-8 pl-11 text-sm text-ink-soft leading-relaxed">
            Blog, Docs und Leitfäden — sichtbar für alle, sobald veröffentlicht. Checklisten darin sind für jede
            lesende Person nur für den eigenen Besuch anklickbar und werden nie gespeichert.
        </p>

        <button
            type="button"
            wire:click="createArticle"
            class="mb-6 rounded-card bg-forest px-4 py-2 text-sm font-medium text-white transition hover:brightness-110 active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-forest focus-visible:ring-offset-2 focus-visible:ring-offset-surface"
        ...
        <div class="space-y-2">
            @forelse ($this->articles as $article)
                ...
                @endphp
                <div wire:key="article-row-{{ $article->id }}" class="flex items-start justify-between gap-3 rounded-card border border-line bg-surface p-4 shadow-map sm:p-5">
                    <button type="button" wire:click="startEdit({{ $article->id }})" class="min-w-0 flex-1 text-left">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span @class([
                                ...
                                'bg-forest-soft text-forest' => $article->is_published,
                                'bg-line text-ink-faint' => ! $article->is_published,
                            ])>{{ $article->is_published ? 'Veröffentlicht' : 'Entwurf' }}</span>
                        </div>
                        <p class="mt-1.5 text-sm font-medium text-ink">{{ $article->title !== '' ? $article->title : 'Ohne Titel' }}</p>
                        <p class="mt-1 text-xs text-ink-faint">
                            {{ $article->author?->name ? 'von '.$article->author->name.' · ' : '' }}
                            {{ $article->is_published && $article->published_at ? 'veröffentlicht am '.$article->published_at->isoFormat('D.M.YYYY') : 'noch nicht veröffentlicht' }}
                        </p>
                    </button>
            ...
            @empty
                <p class="text-sm text-ink-faint">Noch keine Artikel. Fang mit „+ Neuer Artikel" an.</p>
            @endforelse
        </div>
    @else
        {{-- EDITOR — Schreibfluss-Modus: the chrome below (back arrow, badges,
             publish/delete, type chips, toolbar) fades out shortly after you
             start typing, and returns instantly on any click or mouse
             movement. Only the title and the text itself stay put. A fresh
             wire:key per article resets this Alpine scope (and its timers)
             whenever a different article is opened. --}}
        <div
        ...
        >
            <div :class="chromeHidden ? 'opacity-0 pointer-events-none' : 'opacity-100'" class="mb-5 flex items-center justify-between gap-3 transition-opacity duration-500">
                <div class="flex items-center gap-3">
                    <button type="button" wire:click="backToList" class="grid h-8 w-8 place-items-center rounded-card text-ink-faint transition hover:bg-surface hover:text-ink" aria-label="Zurück zur Liste">
                        <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M15 18l-6-6 6-6"/></svg>
                    </button>
                    <span @class([
                        'rounded-full px-1.5 py-0.5 text-[10px] font-medium leading-none',
            ...
            <div :class="chromeHidden ? 'opacity-0 pointer-events-none' : 'opacity-100'" class="mb-4 flex flex-wrap gap-1.5 transition-opacity duration-500">
                @foreach (\App\Models\Article::TYPES as $key => $meta)
                    ...
                    @endphp
                    <button type="button" wire:click="setType('{{ $key }}')" @class(['rounded-[0.45rem] px-3.5 py-1.5 text-sm transition', $activeClasses => $formType === $key, 'bg-paper text-ink-soft hover:text-ink' => $formType !== $key])>{{ $meta['label'] }}</button>
                @endforeach
            </div>
